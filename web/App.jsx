    ...
    <div>
      <div className="stat-grid">
        {[
          ...
          {label:"Critical Events",val:2,badge:"last 1h",bc:"bg-red",vc:"#DC2626"},
...
// ── Onboarding Wizard ──────────────────────────────────────────────────────
function Wizard({apiFetch,loadSources}){
      ...
      <div className="wizard-card">
        {step===0&&(
          <>
            ...
            <div className="wizard-ftr"><div className="wizard-progress">Step 1 of 4</div><button className="btn btn-primary" onClick={()=>setStep(1)} disabled={!form.apiKey}>Next →</button></div>
          </>
        )}
        {step===1&&(
          <>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <div style={{background:"#F8F7F4",border:"1px solid #E5E7EB",borderRadius:8,padding:14,fontSize:12.5,color:"#374151"}}>
                  ...
                  <div style={{fontFamily:"'JetBrains Mono',monospace",color:"#6B7280"}}>
                    IP: <strong style={{color:"#18181B"}}>192.168.1.100</strong> &nbsp;
            ...
            <div className="wizard-ftr">
              ...
              <button className="btn btn-primary" onClick={()=>setStep(2)} disabled={!form.sourceName}>Next →</button>
            </div>
          </>
        )}
        {step===2&&(
...
// ── Sources Page ───────────────────────────────────────────────────────────
function Sources({sources,setSources,apiFetch,loadSources}){
  ...
  const add=async()=>{
    ...
    try{
      ...
      if(res&&res.ok){const d=await res.json();setSources(p=>[...p,d.source]);}
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div className="mapper-stage">
        {[
          {title:"Sources",sub:"Syslog devices",icon:"📡",bg:"#EFF6FF",nodes:sources.map(s=>({name:s.name,detail:":"+s.port+"/"+s.protocol+" · "+(s.logsPerMin||0)+"/min",status:s.status==="active"?s.errors>0?"warn":"ok":"off",icon:"🔥"}))},
...
function Backup({apiFetch}){
  ...
  const doBackup=async()=>{
    ...
    try{
      ...
      if(res&&res.ok){await loadBackups();}
    }catch{}
    setBacking(false);
  };
  const doDelete=async name=>{
    try{await apiFetch(`/backup/${name}`,{method:"DELETE"});await loadBackups();}catch{}
  };
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      {restoring&&(
        <div className="overlay" onClick={()=>setRestoring(false)}>
          <div className="modal" style={{width:400}}>
            <div className="modal-hdr"><div className="modal-title">Restore Backup</div><button className="modal-x" onClick={()=>setRestoring(false)}>✕</button></div>
            <div className="modal-body">
              <div style={{border:"2px dashed #E5E7EB",borderRadius:10,padding:32,textAlign:"center",color:"#9CA3AF",cursor:"pointer"}}>
                <div style={{fontSize:32,marginBottom:8}}>📦</div>
                <div style={{fontSize:13.5}}>Drop backup zip here</div>
            ...
            <div className="modal-ftr"><button className="btn btn-secondary" onClick={()=>setRestoring(false)}>Cancel</button><button className="btn btn-primary">Restore</button></div>
          </div>
        </div>
      )}
    </div>
...
// ── Destinations ───────────────────────────────────────────────────────────
function Destinations({apiFetch}){
  ...
  const save=async()=>{
    try{await apiFetch("/destination",{method:"POST",body:JSON.stringify({api_key:key,ingest_url:url})});setSaved(true);setTimeout(()=>setSaved(false),2000);}catch{}
  };
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div className="card">
        ...
        <div className="dest-list">
          ...
          <div style={{display:"flex",gap:8}}><button className="btn btn-secondary" onClick={test}>{tested==="testing"?"⏳ Testing…":tested==="success"?"✓ Connected":tested==="fail"?"✗ Failed":"Test Connection"}</button><button className="btn btn-primary" onClick={save}>{saved?"✓ Saved":"Save"}</button></div>
        </div>
      </div>
      <div className="card">
...
function Settings({apiFetch}){
  ...
  const restart=async()=>{try{await apiFetch("/restart",{method:"POST"});}catch{}};
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div className="card">
        <div className="card-header"><div className="card-title">System</div></div>
        <div style={{padding:"0 20px"}}>
          {[{l:"Agent Auto-restart",d:"Restart on failure",v:auto,s:setAuto},{l:"Log Rotation",d:"20MB limit, 5 backups",v:rot,s:setRot}].map((r,i)=>(
            <div className="srow" key={i}><div><div className="slabel">{r.l}</div><div className="sdesc">{r.d}</div></div><button className="toggle" style={{background:r.v?"#22C55E":"#D1D5DB"}} onClick={()=>r.s(v=>!v)}/></div>
          ))}
          <div className="srow"><div><div className="slabel">Restart Agent</div><div className="sdesc">Apply changes</div></div><button className="btn btn-secondary btn-sm" onClick={restart}>Restart</button></div>
...
export default function App(){
  ...
  useEffect(()=>{
    if(!authed)return;
    const t=setInterval(loadSources,15000);
    return()=>clearInterval(t);
  },[authed,loadSources]);
  ...
  const handleLogout=async()=>{
    ...
    localStorage.removeItem("sb_token");localStorage.removeItem("sb_role");
      ...
      <div className="shell">
        <aside className={`sidebar ${collapsed?"collapsed":""}`}>
          ...
          <div className="sb-footer">
            <div className="agent-pill">
              ...
              <div className="agent-txt"><strong>Agent running</strong>scalyr-agent-2</div>
            </div>
          </div>
        </aside>
        <main className={`main ${collapsed?"expanded":""}`}>
          ...
          <div className="content">
            ...
            {page==="wizard"&&<Wizard apiFetch={apiFetch} loadSources={loadSources}/>}
            ...
            {page==="sources"&&<Sources sources={sources} setSources={setSources} apiFetch={apiFetch} loadSources={loadSources}/>}
            {page==="parsers"&&<Parsers apiFetch={apiFetch}/>}
            {page==="destinations"&&<Destinations apiFetch={apiFetch}/>}
            {page==="users"&&<Users apiFetch={apiFetch}/>}
            {page==="backup"&&<Backup apiFetch={apiFetch}/>}
            {page==="settings"&&<Settings apiFetch={apiFetch}/>}
          </div>
        </main>
      </div>
    </>
